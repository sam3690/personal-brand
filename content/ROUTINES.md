...
Never use `git add`/`git commit`/`git push`, and never have a routine merge its own PR.
...
`THURSDAY send`, the branch slug changed to `claude/cold-email-thu-<YYYY-MM-DD of today>`, and
step 8 ending "what is due on **Monday**".
